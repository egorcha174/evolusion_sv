...
<script lang="ts">
  ...
  export let children: any; // For slot content
...
<div class="grid grid-cols-2 items-center gap-4">
  <div>
    <label class="text-sm text-gray-700 dark:text-gray-300 font-medium">{label}</label>
    {#if description}
...
<script lang="ts">
  ...
  export let onUpdate: (path: string, value: any) => void;
</script>

<LabeledInput {label}>
  <div class="flex items-center gap-2">
    <span class="text-xs font-mono text-gray-400 uppercase">{value}</span>
    <input type="color" {value} on:input={(e) => onUpdate(path, e.currentTarget.value)} class="w-8 h-8 p-0 border-none rounded-md cursor-pointer bg-transparent shadow-sm"/>
  </div>
</LabeledInput>
...
<LabeledInput {label}>
  <div class="flex items-center gap-2">
    <input type="range" {min} {max} {step} {value} on:input={(e) => onUpdate(path, parseFloat(e.currentTarget.value))} class="w-full accent-blue-500"/>
...
<script lang="ts">
  ...
  export let themeType: 'light' | 'dark';
  export let colorScheme: ColorScheme;
  export let onUpdate: (path: string, value: any) => void;

  $: scheme = colorScheme[themeType];
  $: pathPrefix = themeType;
