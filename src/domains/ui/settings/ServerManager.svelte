<script lang="ts">
   ...
   let isActiveConnected = $derived(
      (id: string) => $appState.activeServer?.id === id,
   );
   ...
   function handleDelete() {
      ...
      const msg = $t("settings.serverManager.deleteConfirm", {
      ...
      <div class="sidebar">
         ...
         <div class="sidebar-footer">
            <button
               class="icon-action"
               onclick={handleAdd}
               title={$t("settings.serverManager.add")}
            >
      ...
      <div class="content">
         {#if mode === "view" && !selectedServer}
            <div class="empty-state">
               <div class="empty-icon">
                  <iconify-icon icon="mdi:server-network" width="64"
                  ></iconify-icon>
               </div>
         ...
         {:else if (mode === "create" || mode === "edit") && draft}
            <div class="form-state">
               ...
               <div class="form-group">
                  <label for="s-token"
                     >{$t("settings.serverManager.token")}</label
                  >
                  <textarea
                     id="s-token"
