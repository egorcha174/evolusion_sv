...
<script lang="ts">
  ...
  $effect(() => {
     if (!selectedId && $appState.activeServer && servers.some(s => s.id === $appState.activeServer?.id)) {
...
<style>
  ...
  .server-item.active .item-name { color: white; }
  ...
  .server-item.active .item-url { color: rgba(255,255,255,0.8); }
  ...
  .icon-action:hover:not(:disabled) { background: rgba(0,0,0,0.05); }
  .icon-action:disabled { opacity: 0.3; cursor: default; }
  .icon-action.danger:hover:not(:disabled) { color: #ff3b30; background: rgba(255, 59, 48, 0.1); }
  ...
  .close-absolute:hover { color: #1d1d1f; }
  ...
  .empty-state h3 { font-size: 1.2rem; margin: 0 0 0.5rem 0; color: #1d1d1f; }

  /* Forms & Views */
  h3 { margin: 0 0 1.5rem 0; font-size: 1.4rem; color: #1d1d1f; }

  .form-group { margin-bottom: 1.25rem; }
  label { display: block; margin-bottom: 0.5rem; font-weight: 500; font-size: 0.9rem; color: #1d1d1f; }
  input, textarea {
    width: 100%;
    padding: 0.75rem;
    background: #f5f5f7;
  ...
  .view-url { color: #86868b; font-size: 0.95rem; margin-top: 0.25rem; }
  .token-preview {
  ...
  .btn.text { background: transparent; color: #86868b; }
  .btn.text:hover { color: #1d1d1f; background: #f5f5f7; }
  .btn.full { width: 100%; }
