...
export interface GalleryTemplate {
  ...
  version: string;
  previewIcon: string;
  templateStructure: Partial<CardTemplate>;
}
...
/**
 * Base interface for a message received from the Home Assistant WebSocket API.
 */
...
/**
 * A function that handles incoming messages from the Home Assistant WebSocket connection.
 */
export type MessageHandler = (message: HassMessage) => void;

/**
 * Defines the contract for a Home Assistant connection client.
 */
export interface HAConnection {
	...
	setBrightness: (entityId: string, brightness: number) => number;
	setTemperature: (entityId: string, temperature: number) => number;
	fetchStates: () => number;
	fetchDevices: () => number;
	fetchAreas: () => number;
	fetchEntityRegistry: () => number;
	subscribeHistory: (entityId: string, startTime?: string, endTime?: string) => number;
	subscribeWeather: (entityId: string) => number;
}
