import { b as store_get, c as attr_class, d as attr, u as unsubscribe_stores, e as ensure_array_like } from "../../chunks/index2.js";
import { $ as $format, h as haStore } from "../../chunks/runtime.js";
import { e as extractDomain, a as activeTabId } from "../../chunks/store2.js";
...
function DeviceCard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    entity.state === "on" ? store_get($$store_subs ??= {}, "$t", $format)("common.on") : entity.state === "off" ? store_get($$store_subs ??= {}, "$t", $format)("common.off") : entity.state;
    $$renderer2.push(`<div${attr_class("device-card svelte-vmg48c", void 0, { "active": isOn })}${attr("data-domain", domain)}>`);
    {
      $$renderer2.push("<!--[-->");
  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (items.length === 0) {
      ...
      if (store_get($$store_subs ??= {}, "$activeTabId", activeTabId) === "home") {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`No dashboard devices found.`);
      } else {
        $$renderer2.push("<!--[!-->");
        $$renderer2.push(`No devices found for "${escape_html(store_get($$store_subs ??= {}, "$activeTabId", activeTabId).replace("_", " "))}".`);
      }
      $$renderer2.push(`<!--]--></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
