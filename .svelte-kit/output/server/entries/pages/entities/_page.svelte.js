...
import { $ as $format, h as haStore } from "../../../chunks/runtime.js";
import { e as extractDomain, b as isToggleable, u as uiDashboardState, s as selectFilteredEntities } from "../../../chunks/store2.js";
import "clsx";
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="page-entities svelte-1ipj8go"><div class="page-header svelte-1ipj8go"><h1>${escape_html(store_get($$store_subs ??= {}, "$t", $format)("entities.title"))}</h1></div> `);
    if (store_get($$store_subs ??= {}, "$haStore", haStore).isLoading) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="loading">${escape_html(store_get($$store_subs ??= {}, "$t", $format)("entities.loading"))}</div>`);
    } else {
      $$renderer2.push("<!--[!-->");
      if (store_get($$store_subs ??= {}, "$haStore", haStore).error) {
        ...
        if (store_get($$store_subs ??= {}, "$selectFilteredEntities", selectFilteredEntities).length === 0 && !store_get($$store_subs ??= {}, "$uiDashboardState", uiDashboardState).filters.search && !store_get($$store_subs ??= {}, "$uiDashboardState", uiDashboardState).filters.domain) {
          $$renderer2.push("<!--[-->");
          $$renderer2.push(`<p>${escape_html(store_get($$store_subs ??= {}, "$t", $format)("entities.noEntities"))}</p>`);
        } else {
          $$renderer2.push("<!--[!-->");
          $$renderer2.push(`<div class="controls svelte-1ipj8go"><div class="filters svelte-1ipj8go"><input type="text"${attr("placeholder", store_get($$store_subs ??= {}, "$t", $format)("entities.search"))}${attr("value", searchQuery)} class="search-input svelte-1ipj8go"/> `);
          $$renderer2.select(
            {
          ...
          $$renderer2.push(`</div> <div class="results-count svelte-1ipj8go">${escape_html(store_get($$store_subs ??= {}, "$t", $format)("entities.showing", {
